...
public final class GraphicView implements View {
	...
	private static BufferedImage loadCards() {
		for (var suit : Suit.values()) {
			for (var rank : Rank.values()) {
				var path = "src/fr/uge/azathro/view/assets/cards/" + rank.name().toLowerCase() + "_" + suit.name()
						+ ".png";
				try {
	...
	public void showGameState(GameState gameState, Set<Integer> selectedIndexes, String lastCombination,
		...
		if (context == null)
			return;
		var screen = context.getScreenInfo();
		context.renderFrame(g -> render(g, gameState, selectedIndexes, lastCombination, lastScore, screen.width(),
				screen.height()));
	}

	private void render(Graphics2D graphics2D, GameState gameState, Set<Integer> selectedIndexes,
			String lastCombination, Integer lastScore, int width, int height) {
	...
	private void drawUIHeader(Graphics2D graphics2D, GameState gameState) {
		...
		graphics2D.drawString(
				"Blind: " + gameState.currentBlind().name() + " | Objectif: " + gameState.currentBlind().score(), 30,
				50);
		graphics2D.drawString("Score: " + gameState.playerState().totalScore(), 30, 85);
		graphics2D.drawString(
				"Deck: " + gameState.deck().size() + " | Mains restantes: " + gameState.playerState().handCount(), 30,
				120);
	}
	...
	private void drawHand(Graphics2D graphics2D, GameState gameState, Set<Integer> selectedIndexes, int width,
			int height, int cardWidth, int cardHeight) {
	...
	public void showPlanetDrawn(Planet planet) {
		...
		context.renderFrame(graphics2D -> {
			...
			drawCenteredText(graphics2D, "Planète tirée : " + planet.name(), screen.width(), screen.height(),
					Color.WHITE);
		});
		sleep(1500);
	}
